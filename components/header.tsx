...
export default function Header() {
  ...
  let reduceAnimation = 'always';

  const initialDivAnimation =
    reduceAnimation !== 'always' ? { y: -100, x: '-50%', opacity: 0 } : { y: 0, x: '-50%' };
  const animateDivAnimation = reduceAnimation !== 'always' ? { y: 0, x: '-50%', opacity: 1 } : {};
  ...
  return (
    <header className="relative z-50">
      <motion.div
        ...
        initial={initialDivAnimation}
        animate={animateDivAnimation}
      ></motion.div>
